...
这里选择了几个比较有代表性的状态管理工具进行比较，可以看到 Redux 的下载量非常大，侧面反应了其被使用的应用场景更多，经历过大量应用的考验，基本能够满足所有业务的应用场景，所以最终选择 Redux 来进行讲解，同时其他的指标也作为了考量标准，如下图：

![image](./2.png)

因此优先对使用场景较大的 Redux 进行讲解，希望能够帮助大家对 Redux 有更进一步的了解，以便在日常的业务开发过程中能够快速上手使用和避坑。
...
当我们在项目中引入 Redux 时，就说明当前项目的复杂度较大或者需要多团队协作的场景，这也说明了引入 Redux 其实也需要进行相对麻烦的编码，导致这个问题的原因也正是 Redux 核心库灵活的设计思想，故意设计成非定制化的样子，且不受框架约束，而 Redux 也针对该问题提供了解决方案。
...
从这张图可以发现，数据是单向数据流的，state 的更新需要通过 dispatch 触发 action，并通过 reducer 帮助我们去更新 state，最后数据驱动视图更新这样的一个流程。

下面再来看一下核心代码文件结构都包含哪些内容，如下：
